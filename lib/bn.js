(function (module, exports) {
  ...
  MPrime.prototype.ireduce = function (num) {
    ...
    let cmp = r.bitLength().cmp(this.n) < 0 ? -1 : r.ucmp(this.p);
    if (cmp === 0) {
      r = new BN(0);
    } else if (cmp > 0) {
      r = r.isub(this.p);
  ...
  K256.prototype.split = function split(input, output) {
    ...
    let lower = input.num & BigInt(mask);
    let upper = input.num >> 22n;
  ...
  Red.prototype.legendreSymbol = function legendreSymbol(a, p) {
    const ls = a ** ((p - 1n) >> 1n) % p;
  ...
  Red.prototype.sqrt = function sqrt(a) {
    ...
    // If p % 4 === 3, we can find square root directly
    if (mod % 4n === 3n) {
      let pow = (mod + 1n) >> 1n;
      return a.num ** pow % mod;
    }
    ...
    let z = 2n;
    while (this.legendreSymbol(z, mod) !== -1n) {
      z += 1n;
    }

    let m = s;
    let c = this.pow(z, q, mod);
    let t = this.pow(a.num, q, mod);
    let r = this.pow(a.num, (q + 1n) >> 1n, mod);
    ...
    while (t !== 1n) {
      ...
      let b = this.pow(c, 1n << (m - i - 1n), mod);
      r = (r * b) % mod;
      t = (t * b * b) % mod;
      c = (b * b) % mod;
      m = i;
  ...
  Red.prototype.pow = function pow(a, num) {
    ...
    for (let i = 2; i < wnd.length; i++) {
      wnd[i] = this.mul(wnd[i - 1], a); // Assuming mul method for BigInt exists
    }
    ...
    let start = bitLength % 26;
    if (start === 0) {
      start = 26;
    }

    for (let i = 0; i < bitLength; i++) {
      const bit = (num.num >> BigInt(bitLength - 1 - i)) & 1n;
      if (res !== wnd[0]) {
        res = this.sqr(res); // Assuming sqr method for BigInt exists
      }
      ...
      current |= bit;
      currentLen++;
      if (currentLen !== windowSize && i !== bitLength - 1) continue;
