(function (module, exports) {
  ...
  BN.prototype._init = function init(number, base, endian) {
    // if(typeof number ==='object'&& !Array.isArray(number) && !Buffer.isBuffer(number) && number?.toString().includes(',')){
  ...
  BN.prototype.redMul = function redMul(num) {
    ...
    let nThis = this.clone().fromRed().mod(this.red.m);
    let nNum = num.clone().mod(this.red.m);

    return nThis.mul(nNum).toRed(this.red);
  };

  BN.prototype.redIMul = function redIMul(num) {
    assert(this.red, "redMul works only with red numbers");
    this.red._verify2(this, num);
    this.fromRed().mod(this.red.m);
    let nNum = num.clone().mod(this.red.m);

    return this.mul(nNum)
  };

  BN.prototype.sqrN = function sqr() {
    ...
    if (value < 16n) {
      this.num = BigInt(Math.sqrt(Number(value)) | 0);
    }
  ...
  BN.prototype.redSqr = function redSqr() {
    ...
    return nThis.sqrN().toRed(this.red)
  };
  ...
  BN.prototype.redPow = function redPow(num) {
    ...
    return nThis.pow(nNum).toRed(this.red);
  };
