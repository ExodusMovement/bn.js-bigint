(function (module, exports) {
  ...
  Red.prototype.sqrt = function sqrt(a) {
    ...
    var mod3 = this.m.num & 3n;
    if (mod3 % 2n !== 1n) {
      throw new Error("Assertion failed: mod3 % 2 === 1");
    }

    // Fast case
    if (mod3 === 3n) {
      var pow = (this.m.num + 1n) >> 2n;
      return a.pow(new BN(pow));
    }
    ...
    if (q.isZero()) {
      throw new Error("Assertion failed: q is not zero");
    }
    ...
    while (a.pow(lpow).cmp(nOne) !== 0) {
      z = z.add(nOne);
    }

    var c = a.pow(q);
    var r = a.pow(q.addn(1).iushrn(1));
    var t = a.pow(q);
    var m = s;
    while (t.cmp(new BN(1)) !== 0) {
      var tmp = t;
      var i = 0;
      while (tmp.cmp(new BN(1)) !== 0) {
        tmp = tmp.sqr();
        i++;
      }
      if (i >= m) {
        throw new Error("Assertion failed: i < m");
      }
      var b = c.pow(new BN(1).iushln(m - i - 1));
